...
make_binary_release() {
  ...
  # The Phoenix scripts assume the assembly module is named "${PROJECT}-assembly". Projects that
  # keep a different assembly module name (e.g. phoenix-adapters -> phoenix-ddb-assembly) can
  # override this via the ASSEMBLY_MODULE environment variable.
  local assembly_module="${ASSEMBLY_MODULE:-${PROJECT}-assembly}"
  ...
  for profile in "${profiles[@]}"; do
    ...
    git clean -d -f -x
    kick_gpg_agent
    # Only pass -Dhbase.profile for projects that actually define an hbase profile matrix;
    # for single-artifact projects (empty profile) passing an empty value is undesirable.
    if [ -z "$profile" ]; then
    ...
    local f_bin_prefix="./${assembly_module}/target/${base_name}"
