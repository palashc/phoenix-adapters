...
if [[ "$1" == "tag" ]]; then
  ...
  shopt -s nocasematch
  if [[ "${PROJECT}" == "phoenix-queryserver" || "${PROJECT}" == "phoenix-connectors" || "${PROJECT}" == "phoenix-thirdparty" || "${PROJECT}" == "phoenix-adapters" ]]; then
    # phoenix-adapters shares the PHOENIX JIRA; by convention its "Fix Version" values are
    # prefixed with the sub-project name minus the "phoenix-" prefix, e.g. "adapters-1.0.0".
