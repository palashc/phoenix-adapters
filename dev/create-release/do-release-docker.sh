...
export PROJECT="${PROJECT:-phoenix-adapters}"

# Name of the Maven module that produces the binary distribution tarball. The Phoenix release
# scripts assume "${PROJECT}-assembly"; phoenix-adapters keeps its historical module name, so we
# make this overridable and default it here.
export ASSEMBLY_MODULE="${ASSEMBLY_MODULE:-phoenix-ddb-assembly}"
...
Options:
  ...
  -p [project] project to build; defaults to $PROJECT env var (phoenix-adapters)
...
if [ "${HOST_OS}" == "DARWIN" ]; then
  run_silent "Building gpg-agent-proxy image with tag ${IMGTAG}..." "docker-proxy-build.log" \
    docker build --build-arg "UID=${UID}" --build-arg "RM_USER=${USER}" \
        --tag "org.apache.phoenixadapters/gpg-agent-proxy:${IMGTAG}" "${SELF}/mac-sshd-gpg-agent"
fi

run_silent "Building phoenix-adapters-rm image with tag $IMGTAG..." "docker-build.log" \
  docker build --tag "org.apache.phoenixadapters/phoenix-adapters-rm:$IMGTAG" --build-arg "UID=$UID" \
      --build-arg "RM_USER=${USER}" "$SELF/phoenix-adapters-rm"
...
if [ "${HOST_OS}" == "DARWIN" ]; then
  ...
  docker run --rm -p 62222:22 \
     ...
     "${GPG_PROXY_MOUNT[@]}" \
     "org.apache.phoenixadapters/gpg-agent-proxy:${IMGTAG}"
...
cmd=(docker run --rm -ti \
  ...
  --mount "type=bind,src=${WORKDIR},dst=/home/${USER}/phoenix-adapters-rm,consistency=delegated" \
  "${JAVA_MOUNT[@]}" \
  "${GIT_REPO_MOUNT[@]}" \
  "${GPG_PROXY_MOUNT[@]}" \
  "org.apache.phoenixadapters/phoenix-adapters-rm:$IMGTAG")
echo "${cmd[*]}"
"${cmd[@]}"
