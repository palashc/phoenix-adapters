...
usage() {
  ...
  cat << __EOF
phoenix-adapters-vote. A script for standard vote which verifies the following items
...
Usage: ${SCRIPT} -s | --source <url> [-k | --key <signature>] [-f | --keys-file-url <url>] [-o | --output-dir </path/to/use>] [-D property[=value]]
  ...
  -s | --source '<url>'         A URL pointing to the release candidate sources and binaries
                                e.g. https://dist.apache.org/repos/dist/dev/phoenix/phoenix-adapters-<version>RC0/
...
cat << __EOF
Although This tool helps verifying Phoenix Adapters RC build and unit tests,
...
# e.g. .../dev/phoenix/phoenix-adapters-1.0.0RC0/ -> PHOENIX_RC_VERSION=phoenix-adapters-1.0.0RC0
PHOENIX_RC_VERSION=$(tr "/" "\n" <<< "${SOURCE_URL}" | tail -n2)
# Strip the "phoenix-adapters-" prefix and the trailing "RC<n>" to get the bare version, e.g. 1.0.0
PHOENIX_VERSION=$(echo "${PHOENIX_RC_VERSION}" | sed -e 's/RC[0-9]*//g' | sed -e 's/phoenix-adapters-//g')
...
function unzip_from_source() {
    tar -zxvf phoenix-adapters-"${PHOENIX_VERSION}"-src.tar.gz
    cd phoenix-adapters-"${PHOENIX_VERSION}"
}
